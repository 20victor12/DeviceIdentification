// Normalized zero for combination
//#define zero 514
...
void setValues()
{
   ...
   device[0].ShapeIndex=1;
   device[1].ShapeIndex=1;
   device[2].ShapeIndex=3;
...
void findShapeMagnitude() {
  ...
  for(i=1;i<arraySize-1;i++)
   ...
   /* for(i=0;i<3;i++)
  {
    Serial.print("absolute c2");
    Serial.println(abs(peaks[device[i].ShapeIndex]-device[i].ShapeMagnitude));
    if(abs(peaks[device[i].ShapeIndex]-device[i].ShapeMagnitude)<=2)
      device[i].flag2=1;
  }*/
...
void findAverage ()
{
  ...
  Serial.print("Previous Steady State");
  Serial.println(D.previousSteadyState);
