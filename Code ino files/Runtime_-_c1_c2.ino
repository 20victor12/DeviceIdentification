...
void setValues()
{
   ...
   device[2].ShapeMagnitude=815;
   device[3].ShapeMagnitude=617;
...
void Deviceidentification_C2()
  {
    ...
    { if(device[i].flag==1)
    {
      if( (abs(D.ShapeIndex-device[i].ShapeIndex)<=10) && (abs(D.ShapeMagnitude-device[i].ShapeMagnitude)<=28))
      { 
        continue;
        //displayC2();
      }
